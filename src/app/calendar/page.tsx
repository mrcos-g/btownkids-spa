  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const visitBloomResponse = await fetch('/api/visitBloomEvents');
        const {
          docs: { docs: bloomEvents },
        } = await visitBloomResponse.json();
        const formattedBloomData = visitBloomFormatter(bloomEvents);

        const combinedEvents = [...formattedData, ...formattedBloomData];
